...
CREATE DOMAIN cs_json_containment_index_v1 AS text[];
...
CREATE OR REPLACE FUNCTION cs_json_containment_v1_v0_0(col jsonb)
  RETURNS cs_json_containment_index_v1
  LANGUAGE sql IMMUTABLE STRICT PARALLEL SAFE
BEGIN ATOMIC
	SELECT ARRAY(SELECT jsonb_array_elements(col->'jc'))::cs_json_containment_index_v1;
END;

CREATE OR REPLACE FUNCTION cs_json_containment_v1_v0(col jsonb)
  RETURNS cs_json_containment_index_v1
  LANGUAGE sql IMMUTABLE STRICT PARALLEL SAFE
BEGIN ATOMIC
	RETURN cs_json_containment_v1_v0_0(col);
END;

CREATE OR REPLACE FUNCTION cs_json_containment_v1(col jsonb)
  RETURNS cs_json_containment_index_v1
  LANGUAGE sql IMMUTABLE STRICT PARALLEL SAFE
BEGIN ATOMIC
	RETURN cs_json_containment_v1_v0_0(col);
